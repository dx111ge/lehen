...
| Edge framework | Tauri v2 | Native app window, MSI installer support, auto-updater, smaller footprint than Electron |
...
- **Frontend framework inside Tauri.** Likely a modern reactive framework (Svelte, Solid, or React), but the choice depends on the first contributor's preference and the complexity of the Edge UI.
